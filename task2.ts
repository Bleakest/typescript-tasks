...
const normalizeData = (unnormalizedData: PostModel[]) => {
  const formattedData = new Map();
  const sortedById = new Map();
  const sortedByArray: string[] = [];
  for (let i = 0; i < unnormalizedData.length; i++) {
    sortedById.set(unnormalizedData[i].id, unnormalizedData[i]);
    sortedByArray.push(unnormalizedData[i].id);
  }

  formattedData.set("byId", sortedById);
  formattedData.set("allIds", sortedByArray);
